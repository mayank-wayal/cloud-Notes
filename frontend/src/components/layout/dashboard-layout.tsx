...
import { Bell, ChevronDown, FileText, FolderOpen, LayoutDashboard, LogOut, Menu, Search, Settings, Sparkles, UploadCloud, X } from "lucide-react";
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <motion.aside
      initial={{ x: mobile ? -280 : -24, opacity: 0 }}
    ...
    >
      <div className="mb-6 flex items-center justify-between gap-3 rounded-2xl bg-white/[0.03] p-3 ring-1 ring-white/[0.06]">
        ...
        {mobile ? (
          <button className="grid h-9 w-9 place-items-center rounded-md text-slate-400 hover:bg-slate-900 hover:text-white" onClick={() => setIsSidebarOpen(false)} aria-label="Close sidebar">
            <X size={18} />
          </button>
        ) : null}
      </div>
      ...
      <div className="mb-4 rounded-2xl border border-slate-800/40 bg-slate-900/25 px-3 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="truncate text-sm font-medium text-slate-200">{user?.name || "Workspace"}</p>
            <p className="truncate text-xs text-slate-500">{user?.email || "Private library"}</p>
          </div>
      ...
      <section className="min-w-0">
        <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#0B1020]/62 backdrop-blur-2xl">
          <div className="flex h-20 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
            <button className="grid h-11 w-11 place-items-center rounded-2xl border border-slate-800/60 bg-slate-950/70 text-slate-400 lg:hidden" onClick={() => setIsSidebarOpen(true)} aria-label="Open sidebar">
              <Menu size={18} />
            </button>
            <div className="min-w-0">
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-slate-500">Workspace</p>
              <h1 className="truncate text-base font-semibold text-white sm:text-lg">CloudNotes for {user?.name || "you"}</h1>
            </div>

            <form onSubmit={handleSearch} className="hidden h-12 w-full max-w-xl items-center gap-3 rounded-2xl border border-slate-800/60 bg-slate-950/60 px-4 text-sm text-slate-500 shadow-2xl shadow-slate-950/20 transition focus-within:border-indigo-400/60 md:flex">
              <Search size={16} />
              <input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-slate-200 outline-none placeholder:text-slate-500" placeholder="Search notes, files, and activity" />
            </form>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="secondary" onClick={logout} className="h-11 rounded-2xl px-3">
                <LogOut size={16} />
